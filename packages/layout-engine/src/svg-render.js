...
import { ICON_SIZE, BODY_SIZE, BODY_LINE_STEP, GRID_GUTTER, ARROW_HEAD_LENGTH, ARROW_HEAD_HALF_WIDTH, ARROW_COLOR, sizeToPx, } from './tokens.js';
...
function frameRenderState(frame, adapter) {
...
function renderFrameText(frame, rs) {
    if (!rs.textBlocks.length)
        return '';
    const parts = [];
    ...
    for (const [blockIndex, block] of rs.textBlocks.entries()) {
        ...
        for (const spec of block) {
            ...
            if (spec.letterSpacing)
                attrs.push(`letter-spacing="${esc(String(spec.letterSpacing))}"`);
            if (smallCaps)
...
function renderIconPlaceholder(frame, rs) {
    if (!frame.icon)
        return '';
...
function renderIcon(frame, rs, iconMarkupByName) {
    if (!frame.icon)
        return '';
    const iconX = frame._layout.placedX + frame._layout.placedW - rs.padRight - ICON_SIZE;
    const iconY = frame._layout.placedY + rs.padTop;
    ...
    if (!inner)
        return renderIconPlaceholder(frame, rs);
...
function renderFrameGroup(frame, adapter, iconMarkupByName) {
    ...
    if (frame.role === 'separator') {
        parts.push(`<line class="dg-separator" x1="${fmt(frame._layout.placedX)}" y1="${fmt(frame._layout.placedY)}"` +
    ...
    if (rs.dashed)
        rectAttrs.push('stroke-dasharray="8 8"');
...
function routeArrows(arrows, bounds) {
    ...
    for (const arrow of arrows) {
        ...
        if (arrow.layoutPath && arrow.layoutPath.length >= 2) {
            const points = _simplifyPath(arrow.layoutPath.map((p) => [p[0], p[1]]));
            result.push({
                points,
...
function _arrowheadPoints(tipX, tipY, prevX, prevY) {
    const dx = tipX - prevX;
    const dy = tipY - prevY;
...
function renderArrows(routed, adapter) {
    const parts = [];
    for (const arrow of routed) {
        ...
        for (let i = 0; i < shaftPoints.length - 1; i++) {
            ...
            inner.push(`<line x1="${fmt(x1)}" y1="${fmt(y1)}" x2="${fmt(x2)}" y2="${fmt(y2)}"` +
                ` fill="none" stroke="${esc(color)}" stroke-width="1" stroke-miterlimit="10"/>`);
        }
        if (head)
            inner.push(`<polygon points="${head.polyPoints}" fill="${esc(color)}"/>`);
        ...
        if (label && label.length > 0) {
            ...
            if (specs.length > 0) {
                ...
                inner.push(`<text font-family="Ubuntu Sans" text-anchor="middle" dominant-baseline="middle">${tspans.join('')}</text>`);
            }
        }
...
// ---------------------------------------------------------------------------
function renderOverlays(overlays, bounds) {
    const OVERLAY_PAD = 8;
    const parts = [];
    for (const ov of overlays) {
        ...
        if (ov.label) {
            inner.push(`<text font-family="Ubuntu Sans" font-size="14" font-weight="400" fill="#000000">` +
...
export function renderFrameDiagramToSvg(diagram, result, adapter, options) {
