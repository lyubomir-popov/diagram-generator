...
export function strokeWidthForClass(frameClass) {
 ...
 */
export function effectiveResolvedStrokeWidth(frame) {
    // If resolveStyles() has run, trust the resolved values.
    if (frame.resolvedStroke !== undefined) {
        const stroke = frame.resolvedStroke;
        if (stroke === 'none' || stroke === 'transparent')
            return 0;
        if (frame.resolvedStrokeWidth != null && frame.resolvedStrokeWidth > 0)
            return frame.resolvedStrokeWidth;
...
export function applyHighlightParentContrast(frame) {
...
function applyResolvedStyleSnapshot(frame, frameClass) {
...
function applyHeadingChildSnapshot(frame, frameClass) {
    frame.resolvedTextFill = frameClass.textFill;
    frame.resolvedIconFill = frameClass.iconFill;
    frame.resolvedHeadingWeight = frameClass.headingText?.weight;
    frame.resolvedHeadingSmallCaps = frameClass.headingText?.smallCaps;
...
export function applyFrameClass(frame, frameClass) {
