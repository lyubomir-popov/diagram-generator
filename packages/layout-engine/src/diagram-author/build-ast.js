...
function readNumber(record, camel, snake) {
    const value = record[camel] ?? record[snake];
    return typeof value === 'number' ? value : undefined;
}
function readString(record, camel, snake) {
    const value = record[camel] ?? (snake ? record[snake] : undefined);
    return typeof value === 'string' ? value : undefined;
}
function readBoolean(record, camel, snake) {
    const value = record[camel] ?? (snake ? record[snake] : undefined);
    return typeof value === 'boolean' ? value : undefined;
}
function readDirection(record) {
...
function normalizeFrameFields(record) {
    const node = {
        id: String(record.id ?? ''),
    };
    ...
    if (maxWidthChars !== undefined)
        node.maxWidthChars = maxWidthChars;
...
export function normalizeFrameTemplate(record) {
...
export function normalizeFrameNode(value, path) {
...
export function buildFrameIndex(root) {
    if (!root) {
...
export function buildFrameAst(rawRoot) {
