...
- Replacing the deleted Python backend with another server-held session layer would deepen the wrong abstraction. The preview server should serve canonical JSON specs and reference images, not own force graph state.
...
- `preview_server.py` should shrink to static serving for force specs, reference images, and any thin save endpoint that remains necessary during migration.
