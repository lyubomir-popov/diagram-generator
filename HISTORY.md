...
- Follow-up after adversarial review: restored the long-lived browser globals (`saveOverrides`, `performUndo`, `performRedo`, `canUndo`, `canRedo`) as thin shims over the extracted modules so the live Playwright/browser preview contract stayed intact, and removed dead `viewer.html` / `force-viewer.html` template leftovers.
