...
**Snap and guide system (2026-05-22):** Both editors now share snap primitives via `editor-base.js`: `snapEdgeToTarget`, `collectGridSnapTargets`, `collectPeerSnapTargets`, `renderGuideLines`, `clearGuideLines`. Grid editor snaps drag and resize to column/row edges with guide lines. Force editor snaps drag to peer node edges with guide lines. Abstract `EngineAdapter` class in `engine-interface.js` defines the Phase 1 swappable engine contract. Force inspector simplified to 3 fields (Node, Style, Actions). Force nodes have 8-handle resize (corners + midpoints) with 8px grid snap. Both editors have undo/redo stacks (max 50 commands): grid editor uses full-state + override-patch undo; force editor uses per-node command undo covering move/pin, style, resize, and pin toggle, with Ctrl+Z/Ctrl+Shift+Z/Ctrl+Y keyboard shortcuts.
