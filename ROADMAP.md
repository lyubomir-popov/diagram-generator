...
This is the non-negotiable layered architecture for the diagram system. All stages below must conform to it. The architecture is informed by the architectural benchmarks in the sibling `diagram-generator-planning` repo (D2, ELK, Penrose, Structurizr, and five other tools).
...
A layout algorithm survey must evaluate which algorithms the competition uses, which diagram types each serves best, and which are worth porting or integrating. The survey informs which engines to add beyond the existing v3 Frame solver and D3-force port. See `diagram-generator-planning/docs/project-brief/benchmarks/` for per-tool architectural analysis and hard-problem inventory. Each candidate engine must be validated with grid-snap post-processing on real diagrams before committing to production integration.
...
Network boundaries (\"DMZ\" wrapping three servers) are just panels with `border: dotted` – they fit the tree and need no new concept. The genuinely new concept is a **zone**: a grouping that spans across panels and cannot be modelled as tree nesting.

A `zone` is declared semantically (`type: zone` with member IDs), resolved post-layout as a bounding-box overlay with dotted border. Zones and network-boundary panels share the same visual treatment.

**Remaining:**
- `Border.DOTTED` rename lands in Stage 10a (feature 001)
- Zone overlay post-layout rendering
- Contrived test YAML with both boundary panels and zones
