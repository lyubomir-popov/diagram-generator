...
- **Nesting tax.** Every framed container introduces an INSET (8px) offset on each side. When a multi-span wrapper contains child panels, derive each child's `col_width` from the wrapper's content span, not from the outer grid column width: `child_col_width = ((wrapper_outer − 2 × INSET) − (N − 1) × inner_gap) / N − 2 × INSET`. **Never hardcode `col_width` on sub-panels independently** — it will drift when the outer grid changes. See "Nesting and alignment rules" in `DIAGRAM.md`.
