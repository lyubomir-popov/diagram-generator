...
- **Borderless boxes:** when an annotation needs an arrow connection or must match a peer box's height, use `Box(borderless=True)` instead of `Helper`. Borderless boxes participate in grid sizing and arrow anchoring; `Helper` does not. If a `Helper` has an arrow attached and the arrow bends unexpectedly, the fix is to convert it to a borderless box.