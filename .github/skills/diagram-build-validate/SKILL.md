...
- **Nesting tax.** Wrappers introduce INSET padding. Derive child widths from `wrapper_outer − 2 × INSET`, then divide among children and gaps. The formula: `child_col_width = ((wrapper_outer − 2 × INSET) − (N − 1) × inner_gap) / N − 2 × INSET`. See "Nesting and alignment rules" in `DIAGRAM.md`.
