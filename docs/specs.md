...
| 035 Compatible engine switcher | `specs/035-compatible-engine-switcher/` | Draft | One canonical YAML/AST-backed diagram can be previewed through multiple compatible engines, with the UI filtering engine choices by typed compatibility instead of showing every engine for every document. |
