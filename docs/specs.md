...
| 022 Diagram authoring AST | `specs/022-diagram-authoring-ast/` | In progress | Compiler scaffold started in `packages/layout-engine/src/diagram-author/` with `compileDiagramYaml` exported from the package; upcoming slices will add edge normalization, layout grammar, defaults, validation, and lowering. |
...
| 024 ELK interactive node alignment | `specs/024-elk-interactive-node-alignment/` | Draft (next after ELK stabilization) | Native ELK constraints for nudge-to-align; persist under `meta.elk` / per-node keys; re-layout with INTERACTIVE strategies — no SVG hacks. Blocked on elkjs feasibility spike. |
| 025 Multi-engine preview architecture | `specs/025-multi-engine-preview-architecture/` | Draft | Defines the preview engine manifest/capability contract, canonical engine-backed save responses, TS-owned engine metadata, and runtime identity so additional engines stop accumulating in `editor.js`. |
| 026 Preview shell decomposition + TS migration | `specs/026-preview-shell-decomposition-ts-migration/` | Draft | Decomposes `scripts/preview/editor.js` into bounded shell modules and moves substantial preview logic into TS-owned browser modules instead of extending the JS monolith. |
